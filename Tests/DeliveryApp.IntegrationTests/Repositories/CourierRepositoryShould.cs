...
public class CourierRepositoryShould : IAsyncLifetime
{
    ...
    {
        //Arrange
        var courierCreateResult = Courier.Create("Иван", TransportEntity.Pedestrian, Location.Create(1, 1).Value);
    ...
    {
        //Arrange
        var courier1CreateResult = Courier.Create("Иван", TransportEntity.Pedestrian, Location.Create(1, 1).Value);
        courier1CreateResult.IsSuccess.Should().BeTrue();
        var courier1 = courier1CreateResult.Value;
        courier1.SetBusy();

        var courier2CreateResult = Courier.Create("Борис", TransportEntity.Pedestrian, Location.Create(1, 1).Value);
